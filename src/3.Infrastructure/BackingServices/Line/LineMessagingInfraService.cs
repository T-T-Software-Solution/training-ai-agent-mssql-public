...
public class LineMessagingInfraService : ILineMessagingInfraService
{
    ...
    public async Task<bool> SendMessageAsync(string accessToken, string message, string replyTokenString)
    {
        var client = new HttpClient();
        ...
        var response = await client.PostAsJsonAsync($"{_lineOptions.APIBaseUrl}/v2/bot/message/reply", new
        {
            messages = new[] {
                new {
                    type = "text",
        ...
        {
            _logger.LogError("SendMessageAsync failed: {StatusCode} {ReasonPhrase}", response.StatusCode, response.ReasonPhrase);
            return false;
        }
        return true;
    }

    public async Task<LineMessagingAPI.ReplyMessage> PushMessageAsync(string accessToken, string message, string toUserId)
    {
        var client = new HttpClient();
        client.DefaultRequestHeaders.Add("Authorization", $"Bearer {accessToken}");

        _logger.LogInformation("Pushing message: {Message} to user: {ToUserId}", message, toUserId);
        ...
        var result = await client.PostAsJsonAsync($"{_lineOptions.APIBaseUrl}/v2/bot/message/push", payload);

        var responseContent = await result.Content.ReadAsStringAsync();

        _logger.LogInformation("Push message response: {ResponseContent}", responseContent);
        return new LineMessagingAPI.ReplyMessage
        {
            InputJson = JsonDocument.Parse(JsonSerializer.Serialize(payload)),
            OutputJson = JsonDocument.Parse(responseContent) // Assuming the response is the same for output
        };
    }
    ...
    public async Task<LineMessagingAPI.UserProfile> GetLineProfile(string accessToken, string userId)
    {
        var client = new HttpClient();
        client.DefaultRequestHeaders.Add("Authorization", $"Bearer {accessToken}");
        ...
        var response = await client.GetAsync($"{_lineOptions.APIBaseUrl}/v2/bot/profile/{userId}");
        var jsonResponse = await response.Content.ReadAsStringAsync();
        var userProfile = JsonSerializer.Deserialize<LineMessagingAPI.UserProfile>(jsonResponse);

        return userProfile ?? new LineMessagingAPI.UserProfile
        {
            UserId = userId,
            DisplayName = "Unknown",
            PictureUrl = null,
